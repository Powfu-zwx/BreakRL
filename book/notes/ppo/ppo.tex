...
\begin{enumerate}
  \item \textbf{抑制每步更新的「步幅」}：当 $r_t$ 越出 $[1-\varepsilon, 1+\varepsilon]$ 后，该样本在继续越界的方向上不再提供梯度——漂移被\textbf{软性抑制}而非硬性禁止，个别 $r_t$ 仍可能越界（见图~3）；
...
\textbf{为什么 $K$ 轮更新安全了？} 一旦某个样本的 $r_t$ 越出 $[1-\varepsilon, 1+\varepsilon]$，clip 就截断它在继续越界方向上的梯度，$K$ 轮复用同一批数据时的误差累积被压制。注意这是\textbf{软约束}：个别 $r_t$ 仍会越界（图~3），只是没有越界方向的梯度激励去进一步放大它。
...
\text{TRPO}       & \text{高（每批一次自然梯度更新）} & \text{高（KL 约束）} & \text{极复杂（二阶）} \\
