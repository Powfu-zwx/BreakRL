...
训练不收敛时，从**你看到的症状**出发查表。每条失败模式给出根因机制、可复现的消融实验与修复手段；所有实验都来自本教材各章的 Notebook，点击链接即可在线阅读，或在本地重跑验证。建议先看[第 8 条](#atlas-8-offline-loss)：离线损失正常、回报塌缩。[English edition](failure-atlas-en.md)
...
## 8. 离线训练：损失正常，回报却塌缩 {#atlas-8-offline-loss}
...
- **复现**：[Decision Transformer 章](notes/decision-transformer/decision-transformer_experiments.ipynb) Figure 1 右（OOD 目标失效）、Figure 2（三档数据天花板）。
...
- **复现**：[Decision Transformer 章](notes/decision-transformer/decision-transformer_experiments.ipynb) Figure 3（格子世界：离线 Q-learning 拼出 14 步最优路径，DT 在最优目标上 60/90 回合无法到达终点）。
