...
\textbf{Lai \& Robbins 下界（1985）}：任何算法至少 $\Omega(\log T)$，不可能比对数增长更快。达到 $O(\log T)$ 就是渐近最优的。
...
早期大量探索（$t$ 小时 $\varepsilon_t$ 大），后期专注利用。遗憾上界降至 $O(\log T)$，匹配下界。
...
取 $p = 1/t$（随时间固定提高置信度），以概率 $\ge 1-2/t$：
\[
...
\begin{itemize}
  ...
  \item $\log t$ 随 $T$ 极慢增长，保证长期来看探索代价很小。
\end{itemize}
...
汤普森采样的期望累积遗憾也达到 $O(\log T)$，且常数项匹配 Lai \& Robbins 下界——它是\textbf{常数最优}的。在实践中通常表现最好。
...
\text{Decaying-}\varepsilon     & \text{随机均匀，概率衰减} & O(\log T)           & \text{简单改进} \\
