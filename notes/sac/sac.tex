...
其中 $u = \mu_\phi + \sigma_\phi\odot\varepsilon$ 是压缩前的高斯样本。修正项 $-\sum_d \log(1-\tanh^2(u_d))$ 恒非负：忽略它会\textbf{低估}动作概率（log-prob 偏小、熵偏大），且动作越靠近边界（$|a_d|\to 1$）误差越大。
...
\textbf{为什么取最小？} 两个网络的初始化与训练动态不同，估计误差\textbf{不完全相关}；取 $\min$ 相当于系统性偏好两者中更悲观的估计——宁可低估也不高估，阻断了过估计的正反馈。
...
\begin{figure}[!htbp]
  ...
  \caption{双 Q vs 单 Q 消融（Pendulum-v1，3 个随机种子，阴影为 $\pm 1\sigma$）。取 $\min$ 的双 Q 版本回报更高、更稳。}
...
\begin{figure}[!htbp]
  ...
  \caption{软目标更新（$\tau=0.005$）vs 硬拷贝（$\tau=1.0$）消融（Pendulum-v1，3 个随机种子，阴影为 $\pm 1\sigma$）。软更新更平滑。}
...
\begin{figure}[!htbp]
  ...
  \caption{温度消融：自动 $\alpha$ vs 固定 $\alpha=0.2$ vs $\alpha=0$（Pendulum-v1，3 个随机种子，阴影为 $\pm 1\sigma$）。自动调节通常更稳。}
...
\text{方法} & \text{动作空间} & \text{样本效率} & \text{稳定性} & \text{离线数据} \\
\hline
\text{PPO} & \text{离散/连续} & \text{中（on-policy）}  & \text{高} & \text{难以复用} \\
\text{SAC} & \text{连续}     & \text{高（off-policy）} & \text{高} & \text{可复用，需保守化} \\
\end{array}
\]
...
\begin{enumerate}
  ...
  \item \textbf{离线数据复用}：SAC 的 off-policy 结构可以复用历史与离线数据；PPO 的 on-policy 结构很难适配离线数据。注意：复用离线数据不等于可以直接离线训练——裸 SAC 在纯离线数据上同样会因外推误差塌缩，需要 CQL/IQL 式的保守化改造（见下一章）。
\end{enumerate}
